...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  var transformed = {
    ...
    n: index,
    isZerocoinSpend: new bitcore.Script(input.script).chunks[0].opcodenum === bitcore.Opcode.map.OP_ZEROCOINSPEND,
  };
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    ...
    isZerocoinMint: new bitcore.Script(output.script).chunks[0].opcodenum === bitcore.Opcode.map.OP_ZEROCOINMINT,
  };
